...
count_table <- data.frame(species = names(colSums(carab_by_plot_2015[,carab_species])),
                          counts_2015 = unname(colSums(carab_by_plot_2015[,carab_species])),
                          counts_2022 = unname(colSums(carab_by_plot_2022[,carab_species])))
...
total_counts_2015 <- colSums(carab_by_plot_2015[carab_species])

total_counts_2022 <- colSums(carab_by_plot_2022[carab_species])
...
# 46 observed species
# Chao1 estimates 61.981 species. 95% conf. int: 50.907 -> 98.051
...
dist_functional_beta_2015 <- comdist(comm = carab_by_plot_2015_stdz_matrix, # the activity-abundance data for each plot
...
adonis2(dist_functional_beta_2015 ~ carab_by_plot_2015_stdz$Treatment, permutations = 999)

# Do a pairwise test to see if any of the groups are different:
pairwise.adonis(dist_functional_beta_2015, carab_by_plot_2015_stdz$Treatment)

# Now run an Analysis of Multivariate Homogeneity of Group Dispersions:
beta_dispersion_functional_2015 <- betadisper(d = dist_functional_beta_2015, 
                                              group = carab_by_plot_2015_stdz$Treatment, 
                                              type = c("median"))
beta_dispersion_functional_2015
plot(beta_dispersion_functional_2015)
boxplot(beta_dispersion_functional_2015, ylab = "Distance to median", xlab="")
anova(beta_dispersion_functional_2015)

# Now create an NMDS ordination for 2015:
nmds_functional_2015 <- metaMDS(dist_functional_beta_2015, trymax = 500, k = 2)
nmds_functional_2015 # stress is quality of fit
stressplot(nmds_functional_2015)
plot(nmds_functional_2015) 

# plot the 2015 NMDS model by treatment:
ordiplot(nmds_functional_2015, disp = "sites", type = "n", xlim = c(-0.4, 0.4), ylim = c(-0.4, 0.4))
points(nmds_functional_2015, dis = "sites", select = which(carab_by_plot_2015_stdz$Treatment=="Forest"), pch = 15, cex = 1, col = "palegreen4")
points(nmds_functional_2015, dis = "sites", select = which(carab_by_plot_2015_stdz$Treatment=="Salvaged"), pch = 16, cex = 1, col = "brown4")
points(nmds_functional_2015, dis = "sites", select = which(carab_by_plot_2015_stdz$Treatment=="Windthrow"), pch = 17, cex = 1, col = "goldenrod2")
ordiellipse(nmds_functional_2015, carab_by_plot_2015_stdz$Treatment, draw = "lines", col = c("palegreen4", "brown4", "goldenrod2"), 
            lwd = 3, kind = "sd", conf = 0.90, label = FALSE)
legend("topleft", legend = c("Forest", "Salvaged", "Windthrow"),
       pch = c(15, 16, 17), cex = 1, bty = "n", col = c("palegreen4", "brown4", "goldenrod2"))
...
# plot the 2015 NMDS model by area:
ordiplot(nmds_functional_2015, disp = "sites", type = "n", xlim = c(-0.4, 0.4), ylim = c(-0.4, 0.4))
points(nmds_functional_2015, dis = "sites", select = which(carab_by_plot_2015_stdz$Area=="northeast"), pch = 15, cex = 1, col = "blue")
points(nmds_functional_2015, dis = "sites", select = which(carab_by_plot_2015_stdz$Area=="southwest"), pch = 16, cex = 1, col = "purple")
ordiellipse(nmds_functional_2015, carab_by_plot_2015_stdz$Area, draw = "lines", col = c("blue", "purple"), 
            lwd = 3, kind = "sd", conf = 0.90, label = FALSE)
legend("topleft", legend = c("northeast", "southwest"),
       pch = c(15, 16, 17), cex = 1, bty = "n", col = c("blue", "purple"))


# Now the same thing for 2022:
dist_functional_beta_2022 <- comdist(comm = carab_by_plot_2022_stdz_matrix, # the activity-abundance data for each plot
                                     dis = dist, # the distance between each carabid species in trait space
                                     abundance.weighted = T) # mean-pairwise distances will be weighted by species abundances

# Now run a PERMANOVA to test the null hypothesis that the centroids of each 
# treatment group are identical and their dispersions are identical:
adonis2(dist_functional_beta_2022 ~ carab_by_plot_2022_stdz$Treatment, permutations = 999)

# Now run an Analysis of Multivariate Homogeneity of Group Dispersions:
beta_dispersion_functional_2022 <- betadisper(d = dist_functional_beta_2022, 
                                              group = carab_by_plot_2022_stdz$Treatment, 
                                              type = c("median"))
beta_dispersion_functional_2022
plot(beta_dispersion_functional_2022)
boxplot(beta_dispersion_functional_2022, ylab = "Distance to median", xlab="")
anova(beta_dispersion_functional_2022)

# Now create an NMDS ordination for 2022:
nmds_functional_2022 <- metaMDS(dist_functional_beta_2022, trymax = 500, k = 2)
nmds_functional_2022 # stress is quality of fit
stressplot(nmds_functional_2022)
plot(nmds_functional_2022) 

# plot the 2022 NMDS model by treatment:
ordiplot(nmds_functional_2022, disp = "sites", type = "n", xlim = c(-0.4, 0.4), ylim = c(-0.4, 0.4))
points(nmds_functional_2022, dis = "sites", select = which(carab_by_plot_2022_stdz$Treatment=="Forest"), pch = 15, cex = 1, col = "palegreen4")
points(nmds_functional_2022, dis = "sites", select = which(carab_by_plot_2022_stdz$Treatment=="Salvaged"), pch = 16, cex = 1, col = "brown4")
points(nmds_functional_2022, dis = "sites", select = which(carab_by_plot_2022_stdz$Treatment=="Windthrow"), pch = 17, cex = 1, col = "goldenrod2")
ordiellipse(nmds_functional_2022, carab_by_plot_2022_stdz$Treatment, draw = "lines", col = c("palegreen4", "brown4", "goldenrod2"), 
            lwd = 3, kind = "sd", conf = 0.90, label = FALSE)
legend("topleft", legend = c("Forest", "Salvaged", "Windthrow"),
       pch = c(15, 16, 17), cex = 1, bty = "n", col = c("palegreen4", "brown4", "goldenrod2"))
...
# plot the 2022 NMDS model by area:
ordiplot(nmds_functional_2022, disp = "sites", type = "n", xlim = c(-0.4, 0.4), ylim = c(-0.4, 0.4))
points(nmds_functional_2022, dis = "sites", select = which(carab_by_plot_2022_stdz$Area=="northeast"), pch = 15, cex = 1, col = "blue")
points(nmds_functional_2022, dis = "sites", select = which(carab_by_plot_2022_stdz$Area=="southwest"), pch = 16, cex = 1, col = "purple")
ordiellipse(nmds_functional_2022, carab_by_plot_2022_stdz$Area, draw = "lines", col = c("blue", "purple"), 
            lwd = 3, kind = "sd", conf = 0.90, label = FALSE)
legend("topleft", legend = c("northeast", "southwest"),
       pch = c(15, 16, 17), cex = 1, bty = "n", col = c("blue", "purple"))
...
CWMs_2015 <- FD::functcomp(traits[,c("PC1", "PC2", "PC3", 
...
# And for 2022:
CWMs_2022 <- functcomp(traits[,c("PC1", "PC2", "PC3", 
                                 numeric_traits_modified,
                                 "antenna_length_standard",
                                 "Water_affinity", "Flight_capability")], 
                       as.matrix(carab_by_plot_2022_stdz[, carab_species]), CWM.type = "all")

carab_by_plot_2022_stdz_with_CWMs <- cbind(carab_by_plot_2022_stdz, CWMs_2022)

# Export data tables ###########################################################
# I will run the statistical models in another R script, so I need to export 
# the plot-level data tables

#write.csv(carab_by_plot_2015_stdz_with_CWMs, file="Aaron_PNR_formatted_data/PNR2015_carabid_counts_by_plot_standardized.csv", row.names = F)
...
#write.csv(carab_by_plot_2022_stdz_with_CWMs, file="Aaron_PNR_formatted_data/PNR2022_carabid_counts_by_plot_standardized.csv", row.names = F)
